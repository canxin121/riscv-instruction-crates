        ...
        for (extension, variants) in analysis {
            for variant in variants {
                for &isa_base in &variant.isa_bases {
                    match isa_base {
                        ISABase::RV32 => { rv32_extensions.insert(*extension); }
                        ISABase::RV64 => { rv64_extensions.insert(*extension); }
                    }
                }
            }
        }
    ...
    fn generate_extension_random_impls(
        ...
        if !rv32_extensions.is_empty() {
            ...
            let rv32_match_arms = rv32_variants.iter().map(|ext| {
                ...
                let has_rv32_specific = variants.iter().any(|v| 
                    !v.is_shared && v.isa_bases.contains(&ISABase::RV32)
                );
                
                if has_shared && has_rv32_specific {
                    // 既有共享又有特定的指令
                    let specific_enum_name = Ident::new(&format!("RV32{}SpecificInstructions", ext), Span::call_site());
                    let shared_enum_name = Ident::new(&format!("{}SharedInstructions", ext), Span::call_site());
                    quote! {
        ...
        if !rv64_extensions.is_empty() {
            ...
            let rv64_match_arms = rv64_variants.iter().map(|ext| {
                ...
                let variants = analysis.get(ext).unwrap();
                let has_shared = variants.iter().any(|v| v.is_shared);
                let has_rv64_specific = variants.iter().any(|v| 
                    !v.is_shared && v.isa_bases.contains(&ISABase::RV64)
                );
                
                if has_shared && has_rv64_specific {
                    // 既有共享又有特定的指令
                    let specific_enum_name = Ident::new(&format!("RV64{}SpecificInstructions", ext), Span::call_site());
                    let shared_enum_name = Ident::new(&format!("{}SharedInstructions", ext), Span::call_site());
                    quote! {
